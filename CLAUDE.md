...
Forth source constraints for block loading: (1) all lines ≤ 64 chars, (2) no forward references (Forth compiles top-down), (3) mutual recursion needs VARIABLE + EXECUTE deferred pattern, (4) kernel lacks `?DO` — use `DUP 0> IF 0 DO ... LOOP ELSE DROP THEN`, (5) `N+` words for N>2 don't exist — use `N +`, (6) HEX/DECIMAL are NOT IMMEDIATE — they change BASE at runtime, not compile time. See critical lesson below.

Critical HEX/DECIMAL trap (bug #20, 2026-03-15): HEX and DECIMAL are runtime words, not IMMEDIATE. When you write `HEX ... : FOO  640 ... ;`, the `640` is parsed by the OUTER INTERPRETER at compile time using whatever BASE is current when the colon definition is being compiled — NOT when FOO executes. If the vocabulary file starts with `HEX` and a colon definition contains what looks like a decimal literal (e.g., `640`), the parser treats it as hex (0x640 = 1600 decimal). The fix: define numeric constants OUTSIDE colon definitions where BASE is known, then reference the constant name inside. Example: `DECIMAL 640 CONSTANT DEFAULT-XMAX HEX` then use `DEFAULT-XMAX` inside colon defs. This bug produces no error message — the wrong value is silently used at runtime. Code review cannot catch it without tracking the BASE state through the source file. PS2-MOUSE had three instances: `640`/`480`/`12` all parsed as hex inside a colon def that ran after `HEX`.
